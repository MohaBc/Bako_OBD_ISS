...
state = {
    ...
    "mppt_t_status":    None,
    "cabin_t":          None,   # cabin air temp °C
    "cabin_humidity":   None,   # cabin humidity % RH (None = not fitted)
    "cabin_status":     None,
...
def parse_can(id_hex: int, d: bytes) -> None:
    ...
    # ── 0x18D601AA — Cabin interior temperature + humidity ───────────────────
    # Byte 1: cabin air temp  (uint8, offset -40)
    # Byte 2: cabin humidity % RH  (0xFF = not fitted)
    # Byte 3: sensor status
    if id_hex == 0x18D601AA and len(d) >= 3:
        t_cabin = decode_temp(d[0])
        hum_raw = d[1]
        hum     = None if hum_raw == 0xFF else hum_raw
        state["cabin_t"]         = t_cabin
        state["cabin_humidity"]  = hum
        state["cabin_status"]    = d[2]
        state["ext"]["bat_t1"]   = t_cabin  # maps to cabin temp display slot
        return
