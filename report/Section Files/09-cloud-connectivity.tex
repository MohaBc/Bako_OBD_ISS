...
Because the SIM800L HTTP transaction takes up to 5 seconds over GPRS (bearer setup, DNS, TCP handshake, data transfer), it is not feasible to transmit every individual CAN frame to the cloud. Instead, the firmware implements a \textbf{snapshot model}: all incoming frames are decoded and merged into a single \texttt{BMSData} struct that always holds the most recent value for each field. Every 5 seconds, this struct is serialized into a single JSON document and sent as one HTTP POST to the VPS.
...
Wireless transmission is handled by a \textbf{SIM800L GSM/GPRS modem} connected to the ESP32 via HardwareSerial UART1. The modem establishes a GPRS bearer using the carrier's APN, then uses the AT+HTTP* command set to perform a standard HTTP POST to the VPS endpoint.
...
The backend is a single Python file (\texttt{backend/server.py}) running under \textbf{Uvicorn}, an ASGI server. The application framework is \textbf{FastAPI}, which provides automatic OpenAPI documentation, WebSocket support, and asynchronous request handling. The dependencies are minimal by design:
...
\begin{table}[h]
    ...
    \begin{tabular}{lll}
        ...
        \texttt{PORT}          & \texttt{8765}          & TCP port for HTTP and WebSocket traffic \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{images/cloud-pipeline.png}
...
Table~\ref{tab:dashboard-panels} summarizes the information panels displayed by the dashboard.
...
For a production deployment, upgrading to HTTPS requires either a SIM800L module with firmware $\geq$ R14.18 (\texttt{AT+HTTPSSL=1} support) or replacing the modem with a SIM7600/A7670 module that supports native TLS.
...
The JSON body sent by the ESP32 to \texttt{POST /api/ingest} contains the fields listed in Table~\ref{tab:json-schema}. All fields are optional in the sense that the server accepts partial payloads, but the firmware always sends a complete snapshot.

\begin{table}[h]
    \centering
    \caption{ESP32 JSON Payload Fields}
    ...
    \begin{tabular}{lll}
        ...
        \texttt{timestamp}      & string & Network time from \texttt{AT+CCLK?} \\
        ...
        \texttt{pack\_v}        & float  & Pack voltage [V] \\
        \texttt{pack\_current\_a} & float & Pack current [A], + = discharge, $-$ = charge \\
        \texttt{cell\_count}    & int    & Number of non-zero cells decoded \\
        ...
        \texttt{cells}          & object & Per-cell \texttt{\{"mv": int, "status": string\}} map \\
        \texttt{temps}          & object & Per-probe temperature map [°C] \\
        \texttt{avg\_temp}      & float  & Average across active probes [°C] \\
        \texttt{fault\_level}   & int    & 0 = normal, 1 = fault \\
        \texttt{error\_code}    & int    & BMS fault code (see Table~\ref{tab:fault-codes}) \\
        \texttt{fault\_name}    & string & Human-readable fault description \\
        \texttt{status}         & object & Bit-field flags (charging, discharging, ready, etc.) \\
        \texttt{charger}        & object & Charging request limits (V, A, start signal) \\
        \bottomrule
    \end{tabular}
\end{table}
...
The following \texttt{curl} command simulates an ESP32 posting a minimal snapshot to the server during testing:
...
The server then pushes this payload to any browser connected to \texttt{/ws/cloud} within 2 seconds.
