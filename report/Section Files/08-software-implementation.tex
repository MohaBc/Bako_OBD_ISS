...
The SIM800L GSM/GPRS module is driven via \textbf{HardwareSerial UART1} (GPIO 16/17) at 9600 baud. HardwareSerial was chosen over SoftwareSerial because the module's AT+HTTP* responses can arrive as multi-line bursts that SoftwareSerial's byte-at-a-time interrupt handler occasionally corrupts at 9600 baud on a loaded CPU.
...
A known failure mode of the SIM800L is a mid-transaction power brownout caused by the $\sim$1.5 A peak current during LTE transmission exceeding the supply rail's transient capability. When this occurs, the module resets and emits an unsolicited \texttt{"Call Ready"} string in its UART output.
...
\begin{enumerate}
    \item Wait 20 s for the SIM card and LTE stack to re-initialize.
...
The \texttt{server.py} dashboard application required zero modifications. It accepts connections on any local interface, including the 192.168.4.x interface created when the PC joins the BAKO\_SMU network. The only operational change for the user is connecting the PC to the \texttt{BAKO\_SMU} Wi-Fi network before launching the server.
...
\midrule
Left panel &
  SOC ring gauge (colour-coded: green $>$30\%, orange 15--30\%, red $<$15\%); pack voltage (V); pack current (A, signed); remaining capacity (Ah); State of Health (SoH) indicator. \\
\midrule
Central panel &
  19-cell voltage bar chart with colour-coded per-cell thresholds (green: normal, orange: approaching limit, red: fault); minimum, maximum, and average cell voltage displayed below the grid; cell spread indicator (max $-$ min, mV). \\
\midrule
Temperature panel &
  Four BMS temperature probe readings (individual bars); rolling 30-sample average trace; time-series line chart scrolling at 1 Hz; high-temperature threshold line drawn at the alert boundary. \\
\midrule
Right panel &
  Extended sensor readings: DC/DC converter voltage, MPPT controller output current, motor temperature (DS18B20 external probe), handbrake state (Engaged / Released), connection status indicator (Live / Waiting / Disconnected), cloud communication event log. \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Serial mode} --- the operator selects or auto-detects the connected ESP32 USB port and sets the baud rate (default 115200). The server reads newline-delimited CAN frames from the serial stream.
    \item \textbf{Wi-Fi AP mode} --- the operator enters the ESP32 AP address (default 192.168.4.1) and TCP port (default 9000), then clicks Apply. The PC must already be connected to the \texttt{BAKO\_SMU} Wi-Fi network.
\end{itemize}
...
\begin{enumerate}
    ...
    \item The FastAPI server is launched as a subprocess on port 8765.
    ...
    \item The browser is opened automatically at \texttt{http://localhost:8765}.
...
Bluetooth Low Energy (BLE) connectivity was designed to complement the Wi-Fi dashboard by providing a lightweight, phone-friendly monitoring channel for the vehicle operator. The ESP32 chip includes a built-in dual-mode Bluetooth radio supporting both Classic Bluetooth and BLE 4.2 / 5.0, making BLE a zero-hardware-cost addition to the platform.

\subsubsection*{Design Overview}

The proposed BLE implementation uses the ESP32 NimBLE stack (available via the Arduino \texttt{NimBLE-Arduino} library) to expose a custom GATT service. The GATT service is identified by a project-specific 128-bit UUID and contains three characteristics:

\begin{description}
    \item[Battery Status characteristic] Readable and notifiable; encodes pack SOC (uint8, 0--100\%), fault level (uint8), and error code (uint8) in a 3-byte payload. Updated at 2~Hz.
    \item[Pack Data characteristic] Readable; encodes pack voltage (uint16, 0.1~V/bit) and pack current (int16, 0.1~A/bit, offset 0) in a 4-byte payload. Updated at 2~Hz.
    \item[Cell Min/Max characteristic] Readable; encodes minimum and maximum cell voltages (uint16, 1~mV/bit each) in a 4-byte payload. Updated at 1~Hz.
\end{description}

A smartphone application subscribed to the Battery Status notification receives an alert whenever SOC drops below 20\% or a BMS fault code is asserted, allowing the rider to monitor the pack state without opening the full web dashboard.

\subsubsection*{Coexistence with Wi-Fi}

The ESP32 shares its radio between Wi-Fi and BLE using time-division multiplexing managed by the ESP-IDF coexistence driver. In practice, enabling BLE advertising alongside the Wi-Fi AP increases the Wi-Fi beacon interval slightly but does not affect the 9000-port TCP data stream. The firmware configures the coexistence policy to prioritise Wi-Fi when a TCP client is connected, allowing BLE notifications to continue at a reduced rate without dropping any telemetry frame.

\subsubsection*{Implementation Status}

The BLE GATT server design was completed and the characteristic schema validated against the NimBLE GATT API. Full firmware integration, pairing security (BLE passkey), and the companion smartphone application remain as short-term development items listed in the project roadmap (Section~\ref{sec:recommendations}).
