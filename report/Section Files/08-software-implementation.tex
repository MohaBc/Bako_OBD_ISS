...
\begin{description}
    ...
    \item[\texttt{soc\_coulomb}] Accumulated Ah count from 0x18FFE5F4, used for charger control decisions.
\end{description}
...
\begin{enumerate}
    ...
    \item Select that port, compile, and click Upload as normal.
...
\begin{itemize}
    ...
    \item \textbf{Wi-Fi mode} --- the operator enters the ESP32 IP address (default 192.168.4.1) and TCP port (default 9000), then clicks Apply. The PC must already be connected to the \texttt{BAKO\_SMU} Wi-Fi network.
\end{itemize}
...
To improve usability and persistence, all sensor configuration parameters are stored locally in \textbf{EEPROM} memory, allowing settings to remain available after reboot or power cycling. Furthermore, an offline telemetry buffering mechanism was implemented using the ESP32 \textbf{SPIFFS} internal flash filesystem. When the TCP connection becomes unavailable, telemetry data is temporarily stored locally and automatically retransmitted once connectivity is restored.
