...
At every 5-second publish interval, the firmware calls \texttt{buildJSON()} to serialize the current \texttt{BMSData} struct into a nested JSON document using the ArduinoJson v7 library, then calls \texttt{httpPost()} to deliver it to the VPS backend via SIM800L GPRS.
